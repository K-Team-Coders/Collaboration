...
def received(message):
    ...
    bot.send_message(message.chat.id, " Вы получили заказ! Спасибо, что выбрали именно нас. "
                                      "Оцените пожалуйста работу нашего сервиса и качество доставки от 1 до 5 - это поможет нам стать лучше. ",
                     reply_markup=markup)
    return()
...
def answr(call1):
    answr = types.InlineKeyboardMarkup(row_width=2)
    ...
    def callback_function1(callback_obj1):
        if callback_obj1.data == "yes":
            ...
            def answr1(message1):
                ...
                requests.post('http://26.200.185.61:8080/addTelegramReview/',
                              json={
                                  "usertext":usertext,
                                  "mark":mark,
                                  "adress":0,
            ...
            reviewdata = callback_obj1.message.date
            print(reviewdata)
