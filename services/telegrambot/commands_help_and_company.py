...
def callback_help(call_help):
    if call_help.data == '11':
        ...
        def reply_message(rpl_msg):
            bot.reply_to(rpl_msg, "Я передал Вашу жалобу специалистам, в скором времени это исправят")
            mark = 1.0
            class_number = 1

            text_help = rpl_msg.text
        ...
        @bot.message_handler(content_types=['text'])
        def reply_message(rpl_msg_two):
            bot.reply_to(rpl_msg_two, "Я передал Вашу жалобу специалистам, в скором времени это исправят")
            mark = 1.0
            class_number = 2
            text_help = rpl_msg_two.text
            sendReview(mark, usertext=text_help, classnumber=class_number)
        ...
        @bot.message_handler(content_types=['text'])
        def reply_message(rpl_msg_three):
            bot.reply_to(rpl_msg_three, "Я передал Вашу жалобу специалистам, в скором времени это исправят")
            mark = 1.0
            class_number = 3
            text_help = rpl_msg_three.text
            sendReview(mark, usertext=text_help, classnumber=class_number)
        ...
        @bot.message_handler(content_types=['text'])
        def reply_message(rpl_msg_four):
            bot.reply_to(rpl_msg_four, "Я передал Вашу жалобу специалистам, в скором времени это исправят")
            mark = 1.0
            class_number = 4
            text_help = rpl_msg_four.text
            sendReview(mark, usertext=text_help, classnumber=class_number)
